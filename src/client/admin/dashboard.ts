import { initFirebase, logout } from '../../lib/firebase-client';
...
async function loadStats() {
  ...
  // Red Flag 분포 바 차트
  const chartEl = document.getElementById('flag-chart')!;
  const total = Object.values(data.redFlagBreakdown).reduce((a, b) => a + b, 0) || 1;
  chartEl.innerHTML = Object.entries(data.redFlagBreakdown).map(([level, count]) => `
    <div class="flag-bar">
